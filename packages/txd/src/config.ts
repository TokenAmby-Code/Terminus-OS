...
// bind) is env/config-driven. A JSON file pointed at by TXD_CONFIG wins;
...
const HARD_DEFAULTS = {
  ...
  dbPath: process.env.HOME ? `${process.env.HOME}/runtimes/database/txd.events.sqlite` : undefined,
  tmuxSocket: 'k12',
} as const;
...
export function assertConfig(raw: PartialConfig): DaemonConfig {
  ...
  if (!cfg.dbPath || !cfg.dbPath.startsWith('/'))
    throw new Error(`txd config error: dbPath must be an absolute path (got ${cfg.dbPath})`);
