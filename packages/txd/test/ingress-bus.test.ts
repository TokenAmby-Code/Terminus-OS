...
test('a native delivered stop projects assistant content into exactly one comm callback', async () => {
  const { store, d, srv, post } = setup();
  try {
    await d.launch({ seat_id: 'palace:W', schema_version: 8, identity: 'src', persona: 'p1', tint: '#1' });
    await d.launch({ seat_id: 'palace:N', schema_version: 8, identity: 'dst', persona: 'p2', tint: '#2' });
    const ask = await d.comm({ schema_version: 8, source_instance_id: 'src', target: 'dst', message: 'question', ask: true, reply: false });
...
test('duplicate stop deliveries dedupe (act.receipt_deduped), never a second stop_reported', async () => {
  ...
  try {
    await d.launch({ seat_id: 'palace:W', schema_version: 8, identity: 'i1', persona: 'p', tint: '#1' });
    await post(delivery('hook.stop', { instance_id: 'i1', schema_version: 8 }));
    const res = await post(delivery('hook.stop', { instance_id: 'i1', schema_version: 8 }));
    expect(res.status).toBe(200);
...
test('a GHOST stop is acked-not-consumed with zero footprint — refused at admission, lane never wedged', async () => {
  ...
  try {
    const res = await post(delivery('hook.stop', { instance_id: '77f7cfb4-orphan', schema_version: 8 }));
...
test('a delivered hook.user_prompt_submit with comm context asserts delivery via the ruled prompt path', async () => {
  ...
  try {
    await d.launch({ seat_id: 'palace:W', schema_version: 8, identity: 'src', persona: 'p1', tint: '#1' });
    await d.launch({ seat_id: 'palace:N', schema_version: 8, identity: 'dst', persona: 'p2', tint: '#2' });
    const acc = await d.comm({ schema_version: 8, source_instance_id: 'src', target: 'dst', message: 'hi', ask: false, reply: false });
...
test('a NATURAL prompt-submit (no comm-message context) is acked-not-consumed — a daily hook can never wedge the lane', async () => {
  ...
  try {
    await d.launch({ seat_id: 'palace:W', schema_version: 8, identity: 'i1', persona: 'p', tint: '#1' });
    const before = await store.count();
    const res = await post(delivery('hook.user_prompt_submit', { instance_id: 'i1', schema_version: 8 }));
    expect(res.status).toBe(200);
...
test('the membrane still guards what txd INGESTS: a consumed-type payload with a raw tmux id is refused, acked, zero footprint', async () => {
  ...
  try {
    ...
    const before = await store.count();
    const res = await post(
      delivery('hook.stop', { instance_id: 'i1', schema_version: 8, content: 'leaked pane %7' }),
    );
    expect(res.status).toBe(200);
...
test('envelope/contract skew is the ONE loud non-2xx: malformed deliveries and version skew 422', async () => {
  ...
  try {
    ...
    const valid = delivery('hook.stop', { instance_id: 'i1', schema_version: 8 });
