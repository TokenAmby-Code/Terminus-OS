...
test('the frame pattern is global — a second frame can never be dropped again', async () => {
  ...
  const pattern = source.match(/const TX_COMM_FRAME = (\/.*\/)([a-z]*);/);
  expect(pattern).not.toBeNull();
  const flags = pattern![2]!;
