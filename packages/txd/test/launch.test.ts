...
test('missing attestation refuses before pane creation or binding', async () => {
  ...
  const res = await d.launch({ seat_id: 'somnium:NE', schema_version: 7, identity: 'i1', persona: 'p' }); // tint missing
...
test('exact repeat launch is successful and appends no duplicate event', async () => {
  const { store, d } = setup();
  const launch = { seat_id: 'palace:W', schema_version: 7, identity: 'i1', persona: 'salamander', tint: '#302800' };
...
test('occupied seat refuses a different instance without tmux or event mutation', async () => {
  ...
  const tmux = new FakeTmux();
  const d = new Daemon(store, tmux);
  await d.launch({ seat_id: 'palace:W', schema_version: 7, identity: 'i1', persona: 'salamander', tint: '#302800' });
  const beforeEvents = await store.count();
  const beforeSeats = await tmux.listSeats();

  const refused = await d.launch({ seat_id: 'palace:W', schema_version: 7, identity: 'i2', persona: 'custodes', tint: '#c9a227' });
...
test('same instance with changed attestations is not an exact repeat', async () => {
  ...
  const refused = await d.launch({ seat_id: 'palace:W', schema_version: 7, identity: 'i1', persona: 'custodes', tint: '#c9a227' });
...
test('one instance cannot bind to multiple seats', async () => {
  ...
  const refused = await d.launch({ seat_id: 'somnium:NE', schema_version: 7, identity: 'i1', persona: 'salamander', tint: '#302800' });
...
test('full attestation tuple hands over with ONE atomic bound event', async () => {
  const { store, d } = setup();
  const res = await d.launch({ seat_id: 'palace:W', schema_version: 7, identity: 'i1', persona: 'salamander', tint: '#302800' });
