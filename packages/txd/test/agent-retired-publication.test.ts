...
test('a forced pane reset over a bound seat publishes agent.retired with cause estate_reset', async () => {
  ...
  const res = await d.resetEstateScope({ schema_version: 14, force: true, scope: 'pane', pane: 'palace:N' });
...
test('a clean stand-down then a pane reset never double-publishes', async () => {
  ...
  expect((await d.resetEstateScope({ schema_version: 14, force: true, scope: 'pane', pane: 'palace:E' })).ok).toBe(true);
...
test('an unregistered daemon (no bus runtime) closes without publishing', async () => {
  const store = new MemoryEventStore();
  const tmux = new FakeTmux();
  const d = new Daemon(store, tmux, undefined, undefined, null, null, null, null, undefined, retirementClear);
