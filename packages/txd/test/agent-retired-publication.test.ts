...
test('a forced pane reset over a bound seat publishes agent.retired with cause estate_reset', async () => {
  ...
  const res = await d.resetEstateScope({ schema_version: 13, force: true, scope: 'pane', pane: 'palace:N' });
...
test('a clean stand-down then a pane reset never double-publishes', async () => {
  ...
  expect((await d.resetEstateScope({ schema_version: 13, force: true, scope: 'pane', pane: 'palace:E' })).ok).toBe(true);
