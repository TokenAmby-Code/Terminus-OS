...
test('mutation ingress refuses a raw tmux id IN AN IDENTIFIER FIELD, before tmux or persistence', async () => {
  ...
  const attacks: Array<{ path: string; body: Record<string, unknown>; error: string }> = [
    { path: '/agents/launch', error: 'invalid_launch_request', body: { seat_id: '%91', schema_version: 12, identity: 'i1', persona: 'p', tint: '#1' } },
    { path: '/agents/launch', error: 'invalid_launch_request', body: { seat_id: 'palace:W', schema_version: 12, identity: '@22', persona: 'p', tint: '#1' } },
    { path: '/agents/close', error: 'invalid_close_request', body: { source_agent_id: 'ov-1', targets: ['$7'], schema_version: 12 } },
    { path: '/agents/close', error: 'invalid_close_request', body: { source_agent_id: '%3', targets: ['palace:W'], schema_version: 12 } },
  ];
  ...
  try {
    for (const message of ['pin zod@4.4', 'the positional $1', 'it cost $20', 'attesting from pane %28']) {
      const res = await fetch(`http://127.0.0.1:${srv.port}/agents/comm`, {
        method: 'POST',
        body: JSON.stringify({ schema_version: 12, source_agent_id: 'ov-1', target: 'palace:W', message }),
      });
...
test('handler errors are sanitized before structured logging', async () => {
  ...
  try {
    ...
    const res = await fetch(`http://127.0.0.1:${srv.port}/agents/launch`, {
      method: 'POST', body: JSON.stringify({ seat_id: 'palace:W', schema_version: 12, identity: 'i1', persona: 'p', tint: '#1' }),
    });
...
test('no tmux id appears in any /agents/*, /tmux/read, or /ctl response', async () => {
  ...
  try {
    ...
    bodies.push(await (await post('/agents/launch', { seat_id: 'somnium:NE', schema_version: 12, identity: 'i1', persona: 'p', tint: '#1' })).json());
    bodies.push(await (await post('/agents/send', { target: 'somnium:NE', text: 'hello', schema_version: 12 })).json());
...
test('a comm-wait response carrying an agent reply that quotes a tmux id breaches on the way out', async () => {
  ...
  try {
    const res = await fetch(`http://127.0.0.1:${srv.port}/agents/comm/wait`, {
      method: 'POST',
      body: JSON.stringify({ schema_version: 12, ask_id: 'ask-1', subscriber_agent_id: 'ov-1' }),
    });
