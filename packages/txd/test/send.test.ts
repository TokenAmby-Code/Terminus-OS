...
async function boundSeat(d: Daemon, seat: string, identity: string) {
  await d.launch({ seat_id: seat, schema_version: 6, identity, persona: 'salamander', tint: '#302800' });
}
...
test('bare pane, operator idle → enqueue-by-default then delivered', async () => {
  ...
  const res = (await d.send({ target: 'somnium:NE', text: 'hello', schema_version: 6 })) as SendReceipt;
...
test('unresolved target REFUSED at admission — never gated, never enqueued', async () => {
  const { store, d } = setup();
  const res = await d.send({ target: 'ghost:X', text: 'hi', schema_version: 6 });
...
test('receipt carries the send OWN resolution (never re-derived) — bound_seq parity', async () => {
  ...
  const res = (await d.send({ target: 'i-42', text: 'yo', schema_version: 6 })) as SendReceipt;
  expect(res.verdict).toBe('delivered');
...
class PartialTmux extends FakeTmux {
  override async sendToSeat(_seatId: string, text: string) {
    return {
      ...
      verdict: 'partial_delivered' as const,
      trace: [
        { kind: 'literal_insert' as const, attempt: 1, ok: true },
        { kind: 'submit_enter' as const, attempt: 1, ok: true },
        { kind: 'submit_verify' as const, attempt: 1, ok: false },
      ],
    };
  }
}
...
test('partial delivery (inserted, not submitted) → partial_delivered carries bytes, stays enqueued', async () => {
  ...
  const submitEvents = (await store.readAll()).filter((e) => e.event_type === 'act.send_submit_observed');
  expect(submitEvents.map((e) => e.payload)).toEqual([
    expect.objectContaining({ kind: 'literal_insert', attempt: 1, ok: true }),
    expect.objectContaining({ kind: 'submit_enter', attempt: 1, ok: true }),
    expect.objectContaining({ kind: 'submit_verify', attempt: 1, ok: false }),
  ]);
  await store.close();
});
