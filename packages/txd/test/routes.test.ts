...
test('the bus door serves hook.stop deliveries with the ruled stop behavior', async () => {
  ...
  try {
    const res = await fetch(`http://127.0.0.1:${srv.port}/ingress/bus`, {
      method: 'POST',
      body: JSON.stringify(delivery('hook.stop', { instance_id: 'i1', schema_version: 8 })),
    });
...
test('GET /tmux/read/estate serves the estate view including who is bound', async () => {
  const d = daemon();
  await d.launch({ seat_id: 'somnium:NE', schema_version: 8, identity: 'i1', persona: 'salamander', tint: '#302800' });
  const srv = makeServer({ bind: '127.0.0.1', port: 0, daemon: d, build, machine: 'test' });
  try {
...
test('POST /ctl/estate/rotate resets a page in-process instead of killing the estate server', async () => {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${srv.port}/ctl/estate/rotate`, {
      ...
      body: JSON.stringify({ schema_version: 8, force: true, scope: 'page', page: 'somnium' }),
    });
...
test('POST /ingress/tmux reconstructs the canonical page after a pane exits', async () => {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${srv.port}/ingress/tmux`, {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ schema_version: 8, event: 'pane-exited', page: 'palace' }),
    });
    expect(response.status).toBe(200);
...
test('comm identity ambiguity is a loud typed refusal with zero communication effects', async () => {
  ...
  await d.launch({ seat_id: 'palace:W', schema_version: 8, identity: 'source', persona: 'source-persona', tint: '#1' });
  await d.launch({ seat_id: 'palace:N', schema_version: 8, identity: 'a', persona: 'astartes', tint: '#2' });
  await d.launch({ seat_id: 'palace:S', schema_version: 8, identity: 'b', persona: 'astartes', tint: '#3' });
  const before = await store.count();
  const srv = makeServer({ bind: '127.0.0.1', port: 0, daemon: d, build, machine: 'test' });
  try {
    const response = await fetch(`http://127.0.0.1:${srv.port}/agents/comm`, {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ schema_version: 8, source_instance_id: 'source', target: 'astartes', message: 'must not land', ask: false, reply: false }),
    });
    expect(response.status).toBe(422);
...
test('adversarial: every legacy route is dead (404) — no shim, no alias', async () => {
  const d = daemon();
  await d.launch({ seat_id: 'somnium:NE', schema_version: 8, identity: 'i1', persona: 'p', tint: '#1' });
  const srv = makeServer({ bind: '127.0.0.1', port: 0, daemon: d, build, machine: 'test' });
  try {
    for (const [method, path] of LEGACY) {
      const res = await fetch(`http://127.0.0.1:${srv.port}${encodeURI(path)}`, {
        method,
        ...(method === 'POST' ? { body: JSON.stringify({ schema_version: 8 }) } : {}),
      });
  ...
  try {
    for (const hook of HOOK_TYPES) {
      const res = await fetch(`http://127.0.0.1:${srv.port}/ingress/hooks/${hook}`, {
        ...
        body: JSON.stringify({ instance_id: 'i1', schema_version: 8 }),
      });
      expect(res.status).toBe(404);
    }
