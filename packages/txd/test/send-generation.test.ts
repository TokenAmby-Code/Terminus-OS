...
for (const scenario of ['disappeared', 'changed generation', 'retired', 'moved'] as const) {
  test(`frozen bound send is cancelled when its binding ${scenario}`, async () => {
    ...
    await daemon.launch({ seat_id: SEAT, schema_version: 8, identity: 'occupant-A', persona: 'astartes', tint: '#302800' });
    ...
    const receipt = (await daemon.send({ target: 'occupant-A', text: 'frozen for A', schema_version: 8 })) as SendReceipt;
