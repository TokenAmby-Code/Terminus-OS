...
async function setup() {
  ...
  await daemon.launch({ seat_id: 'palace:W', schema_version: 9, identity: 'source', persona: 'custodes', tint: '#1' });
  await daemon.launch({ seat_id: 'somnium:N', schema_version: 9, identity: 'target-a', persona: 'pax', tint: '#2' });
  await daemon.launch({ seat_id: 'somnium:S', schema_version: 9, identity: 'target-b', persona: 'astartes', tint: '#3' });
  return { store, daemon };
}
...
test('identity trinity resolves instance, singleton persona, and canonical pane with one frozen target', async () => {
  for (const identity of ['target-a', 'pax', 'somnium:N']) {
    ...
    const result = await daemon.comm({ schema_version: 9, source_instance_id: 'source', target: identity, message: 'hello', ask: false, reply: false });
...
test('absence and ambiguity refuse before communication effects', async () => {
  ...
  await expect(daemon.comm({ schema_version: 9, source_instance_id: 'source', target: 'absent', message: 'x', ask: false, reply: false })).rejects.toThrow('identity_absent');
  expect(await store.count()).toBe(before);
  await daemon.launch({ seat_id: 'palace:E', schema_version: 9, identity: 'other-pax', persona: 'pax', tint: '#4' });
  const beforeAmbiguous = await store.count();
  await expect(daemon.comm({ schema_version: 9, source_instance_id: 'source', target: 'pax', message: 'x', ask: false, reply: false })).rejects.toThrow('identity_ambiguous');
...
test('page resolution is an immutable exact inventory and payload stays opaque', async () => {
  const { store, daemon } = await setup();
  const message = '---\na: 1\n---\n{"x":"λ\\n\\\""}';
  const result = await daemon.comm({ schema_version: 9, source_instance_id: 'source', page: 'somnium', message, ask: false, reply: false });
...
test('delivery assertion is asynchronous, replay-safe, and echoes separately', async () => {
  const { store, daemon } = await setup();
  const sent = await daemon.comm({ schema_version: 9, source_instance_id: 'source', target: 'target-a', message: 'x', ask: false, reply: false });
  expect((await store.readAll()).some((e) => e.event_type === 'act.comm_delivery_asserted')).toBe(false);
  expect(await daemon.promptSubmitted({ schema_version: 9, instance_id: 'target-a', message_id: sent.message_id })).toEqual({ ok: true, asserted: true });
  expect(await daemon.promptSubmitted({ schema_version: 9, instance_id: 'target-a', message_id: sent.message_id })).toEqual({ ok: true, asserted: false });
...
test('one-way communication never subscribes its sender or contaminates a later ask callback', async () => {
  const { store, daemon } = await setup();
  await daemon.comm({ schema_version: 9, source_instance_id: 'source', target: 'target-a', message: 'one-way', ask: false, reply: false });
  const ask = await daemon.comm({ schema_version: 9, source_instance_id: 'source', target: 'target-a', message: 'real ask', ask: true, reply: false });
...
test('explicit reply wins over stop fallback and produces exactly one callback', async () => {
  const { store, daemon } = await setup();
  const ask = await daemon.comm({ schema_version: 9, source_instance_id: 'source', target: 'target-a', message: 'question', ask: true, reply: false });
  await daemon.comm({ schema_version: 9, source_instance_id: 'target-a', message: 'answer', ask: false, reply: true });
  await daemon.commStop('target-a', 'fallback', 'stop-1', null);
  const result = await daemon.waitComm({ schema_version: 9, ask_id: ask.ask_id!, subscriber_instance_id: 'source', timeout_ms: 420_000 });
...
test('stop fallback aggregates one replay-safe callback per page target', async () => {
  const { store, daemon } = await setup();
  const ask = await daemon.comm({ schema_version: 9, source_instance_id: 'source', page: 'somnium', message: 'report', ask: true, reply: false });
...
test('overlapping subscriptions to one stop and subscriber collapse to one assertion', async () => {
  const { store, daemon } = await setup();
  const first = await daemon.comm({ schema_version: 9, source_instance_id: 'source', target: 'target-a', message: 'one', ask: true, reply: false });
  const second = await daemon.comm({ schema_version: 9, source_instance_id: 'source', target: 'target-a', message: 'two', ask: true, reply: false });
  await daemon.commStop('target-a', 'single stop content', 'shared-stop-event', null);
  expect((await store.readAll()).filter((e) => e.event_type === 'act.comm_callback_asserted')).toHaveLength(1);
  expect((await daemon.waitComm({ schema_version: 9, ask_id: first.ask_id!, subscriber_instance_id: 'source', timeout_ms: 420_000 })).complete).toBe(true);
  expect((await daemon.waitComm({ schema_version: 9, ask_id: second.ask_id!, subscriber_instance_id: 'source', timeout_ms: 420_000 })).complete).toBe(true);
});

test('reply targets the latest inbound sender without cancelling an older ask fallback', async () => {
  const { daemon } = await setup();
  const older = await daemon.comm({ schema_version: 9, source_instance_id: 'target-a', target: 'source', message: 'ask', ask: true, reply: false });
  await daemon.comm({ schema_version: 9, source_instance_id: 'target-b', target: 'source', message: 'newest', ask: false, reply: false });
  const reply = await daemon.comm({ schema_version: 9, source_instance_id: 'source', message: 'to B', ask: false, reply: true });
  expect(reply.targets[0]!.instance_id).toBe('target-b');
  await daemon.commStop('source', 'fallback to A', 'stop-source', null);
  expect((await daemon.waitComm({ schema_version: 9, ask_id: older.ask_id!, subscriber_instance_id: 'target-a', timeout_ms: 420_000 })).callbacks[0]!.content).toBe('fallback to A');
});
