...
test('present WITHIN window → gated; window echoed in the decision', async () => {
  ...
  const res = (await d.send({ target: SEAT, text: 'hi', schema_version: 7 })) as SendReceipt;
...
test('continuously active bound agent pane delivers immediately', async () => {
  ...
  const res = (await d.send({ target: SEAT, text: 'report', schema_version: 7 })) as SendReceipt;
...
test('recent operator input on an unbound pane holds, then releases on guard expiry', async () => {
  ...
  const res = (await d.send({ target: SEAT, text: 'held', schema_version: 7 })) as SendReceipt;
  expect(res.verdict).toBe('enqueued_gated');
...
test('held message for occupant A is cancelled before replacement occupant B can receive it', async () => {
  ...
  const held = (await d.send({ target: SEAT, text: 'only for A', schema_version: 7 })) as SendReceipt;
  ...
  await d.launch({ seat_id: SEAT, schema_version: 7, identity: 'occupant-B', persona: 'astartes', tint: '#302800' });
  nowMs += SEND_PRESENCE_ACTIVITY_WINDOW_MS + 1;
  await release!();
