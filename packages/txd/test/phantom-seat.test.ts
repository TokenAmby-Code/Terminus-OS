...
test('a cleared seat whose pane is gone is flagged as a phantom', async () => {
  ...
  await d.launch({
    seat_id: 'proof:bus', schema_version: 8, identity: 'i-1', persona: 'astartes', tint: '#101010',
  });
  await d.close({ target: 'proof:bus', schema_version: 8 });
...
test('an observable dead pane is not reported as absent', async () => {
  const { tmux, d } = setup();
  await d.launch({
    seat_id: 'palace:W', schema_version: 8, identity: 'i-1', persona: 'salamander', tint: '#302800',
  });
