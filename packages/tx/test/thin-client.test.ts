...
test('public commands are declarative endpoint pingers with no parallel state surface', () => {
  expect(COMMANDS.map((command) => command.path.join(' '))).toEqual([
    'comm', 'journal dispose', 'inspect hooks', 'comm delivery', 'run', 'close', 'mode', 'clipboard push', 'clipboard pull',
    'estate compact-events', 'estate show', 'estate zombies', 'estate reconcile', 'estate abandon', 'estate event', 'estate rotate',
  ]);
});
