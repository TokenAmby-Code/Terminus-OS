...
test("published replay JSON Schema and OpenAPI stay pinned to the runtime contract", async () => {
  ...
  for (const path of ["/v1/replays/admit:", "/v1/replays/{replay_id}:", "/v1/replays/{replay_id}/events:", "/ingress/github:"]) {
    expect(openapi).toContain(path);
  }
  for (const contract of ["ReplayAdmission:", "ReplayEventInput:", "ReplayEventRecord:", "ReplayProjection:", "GithubEventName:"]) {
