...
const forbidden = ["nas-path.sh", "imperium_config", "imperium_cfg", "_IMPERIUM_CFG_", "_IMPERIUM_TOKEN_API_HOST"];
  ...
  test("tracked files do not reference legacy registry surfaces", () => {
    ...
    for (const path of result.stdout.toString().split("\0").filter(Boolean)) {
      ...
      for (const token of forbidden) {
        if (content.includes(Buffer.from(token))) violations.push(`${path}: forbidden Token-OS surface ${JSON.stringify(token)}`);
      }
    }
